...
function ButtonGroupText({ className, asChild = false, ...props }: ComponentProps<"div"> & { asChild?: boolean }) {
  ...
  return (
    <Comp
      className={cn(
        "bg-muted flex items-center gap-2 rounded-md border px-4 text-label-md font-medium [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none",
        className
      )}
      {...props}
    />
