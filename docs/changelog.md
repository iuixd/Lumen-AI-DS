...
- Removed Inter and Roboto Mono from the type scale system-wide, at direct user request ("remove both fonts").
  - Affected token: `fontFamily.sans`/`brand`/`interface`/`mono`/`documentation-mono` (`packages/tokens/src/typography.json`)
  - Source: direct user instruction, **not** a Figma-sourced change — Figma was unreachable at the time (MCP disconnected), so this was not cross-checked against or reconciled back into the Figma file, which (as last read) still documents Inter/Roboto Mono as this system's two base families. Flagged in `typography.json`'s `_comment` as out of sync with Figma pending reconciliation.
  - Previous: `sans`/`brand` were Inter-only (plus generic system fallbacks, no other named webfont); `interface` listed Instrument Sans primary with Inter as its fallback; `mono` was Roboto Mono-only; `documentation-mono` was Space Mono primary with Roboto Mono as its fallback.
  - Current: per user decision, `sans`/`brand` now use Instrument Sans as their primary face (matching `interface`'s existing choice, rather than left with no named font) and `interface` no longer lists Inter as a fallback; `mono` now uses Space Mono, and `documentation-mono` simplified to Space Mono only (its Roboto Mono fallback no longer exists to fall back to). The Storybook Google Fonts `@import` dropped its `Inter:wght@600;700` request entirely and added weight 700 to the existing Instrument Sans request (`400;500;600` → `400;500;600;700`), preserving glyph coverage for the `app-logo`/`app-logo-compact` brand-mark tokens (both weight 700) now that they render in Instrument Sans instead of Inter. Roboto Mono was never loaded as a webfont anywhere in the repo (confirmed: no `@import`/`@font-face` referenced it), so its removal has no webfont-loading impact.
  - Affects: `packages/tokens/src/typography.json`, `packages/tokens/dist/*` (regenerated), `packages/storybook/.storybook/tailwind.css`
  - Migration: visual-only for consumers already using the token classes (`font-sans`/`font-brand`/`font-interface`/`font-mono`/`font-documentation-mono`) — no class/prop names changed. Any consumer CSS hardcoding `"Inter"` or `"Roboto Mono"` directly (bypassing the token layer, against this repo's own hard rules) would need manual update; none was found in this codebase.
  - Validation: `pnpm --filter @lumen/tokens build`, `pnpm --filter @lumen/ui typecheck`/`test` (313/313), repo-wide `pnpm -r typecheck`, `pnpm lint`, `@lumen/web-components`/`@lumen/angular` tests (74/70, unaffected), and `pnpm --filter @lumen/storybook build-storybook` all passed; confirmed no remaining `Inter`/`Roboto Mono` references in the built token CSS or the built Storybook bundle's Google Fonts import.
