...
- Added `@lumen/create-app`, a CLI scaffolder that generates a React + TypeScript + Vite + Tailwind application under `apps/<name>`, wired to `@lumen/tokens`, `@lumen/ui`, and (optionally) `@lumen/patterns` via `workspace:*` dependencies, plus a clean-install CI workflow that proves a generated app installs, type-checks, and builds from a fresh checkout.
  - Source: direct engineering request (not a Figma sync), scoped and confirmed via Expert Review Mode before implementation. Not sourced from any `docs/roadmap.md` phase — no phase currently covers a pnpm-workspace app-scaffolding tool; this is flagged here rather than left unrecorded, since every prior non-Figma addition in this file (`@lumen/web-components`, `@lumen/angular`) cited a roadmap phase and this one has none. `docs/roadmap.md` itself is not updated by this entry — left for a follow-up decision.
  - Previous: `pnpm-workspace.yaml` only globbed `packages/*`; there was no supported way to develop a consumer application and the design system together in one repository — only the Git-dependency model documented under "Use Lumen in a product application" in the root README.
  - Current: `packages/create-app` (`@lumen/create-app`, private) exposes `create-lumen-app` via `dist/cli.js` (built with plain `tsc`, matching this repo's existing hand-rolled-script convention rather than adding a bundler dependency). Root scripts `create:react`/`create:react:dev` run it; `pnpm-workspace.yaml` gained `apps/*`; `apps/.gitkeep` keeps the glob resolvable pre-generation. The CLI validates project name, target-directory emptiness, and workspace-root markers (`pnpm-workspace.yaml`, `packages/tokens/package.json`, `packages/ui/package.json`) before scaffolding; supports both interactive prompts (`@clack/prompts`) and non-interactive flags (`--name`, `--patterns`/`--no-patterns`, `--install`/`--no-install`) for CI use. The generated app renders a small AI-workspace screen composed only from confirmed real `@lumen/ui` exports (`Badge`, `Button`, `Card`/`CardHeader`/`CardTitle`, `Container`, `Input`, `Stack`); its Tailwind config reuses `packages/storybook/tailwind.config.cjs`'s own content-scanning convention, with one addition — `*.test.{ts,tsx}` is excluded, since Tailwind's content scanner regex-tokenizes raw file text and was otherwise misreading a regex literal inside `Button.test.tsx` as a bogus utility-class candidate. Root `typecheck`/`test` are rescoped from `pnpm -r` to `pnpm --filter "./packages/**"`, and `.eslintrc.cjs` gained an `apps` ignore pattern, so a locally-generated (never-committed) app can never be silently picked up by the repository's own root checks. `.github/workflows/react-starter.yml` proves the whole path end to end: frozen-lockfile install → typecheck/test/build the scaffolder → generate an app non-interactively → a second, explicitly `--no-frozen-lockfile` install (required because GitHub Actions' `CI=true` makes pnpm default to frozen-lockfile even for a bare `pnpm install`, which would otherwise fail since the generated app's dependencies are never part of the first, frozen install) → typecheck and build the generated app → confirm expected files exist.
  - Affects: `packages/create-app/**` (new package), `.github/workflows/react-starter.yml` (new), `pnpm-workspace.yaml`, `apps/.gitkeep` (new), `package.json` (root scripts), `.eslintrc.cjs`, `.gitignore`, `README.md` ("Create a React application" section)
  - Migration: none — additive; the root `typecheck`/`test` rescoping only narrows what was already an unbounded `pnpm -r` to the packages that actually exist today, no existing package was excluded
  - Validation: `pnpm lint`, `pnpm typecheck`, and `pnpm test` passed repo-wide; 27 `@lumen/create-app` unit tests passed; a generated app was installed, type-checked, and built successfully both locally and by reproducing the GitHub Actions `CI=true` environment directly (the `--no-frozen-lockfile` fix was verified by first reproducing the failure, then confirming the fix, under `CI=true`)
  - Changeset: none (`@lumen/create-app` is private tooling starting at its own initial `0.1.0`, nothing to version-bump from — same treatment as `@lumen/web-components`/`@lumen/angular`)
