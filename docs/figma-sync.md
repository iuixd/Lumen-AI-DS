...
| KPICard | `1007:3700` canonical `1119:3343`-`45`, re-verified against example `1197:1652` (`1102:6521`-`23`) | `packages/ui/src/primitives/KPICard.tsx`, `packages/web-components/src/kpi-card/lumen-kpi-card.ts`, `packages/angular/src/kpi-card/lumen-kpi-card.ts` | `Primitives/KPICard` (React only — WC/Angular not covered) | Partially Synced — label/value typography rounded to nearest existing type-scale tier; shadow opacity corrected 0.04→0.08 against the canonical instance; success-tint bg color still observed as a near-but-not-exact match, not re-verified; React/WC/Angular parity complete | 2026-07-20 |
| Theme Toggle | `1197:1652` (Header instance) | `packages/ui/src/primitives/ThemeToggle.tsx`, `packages/web-components/src/theme-toggle/lumen-theme-toggle.ts`, `packages/angular/src/theme-toggle/lumen-theme-toggle.ts` | `Primitives/ThemeToggle` (React only — WC/Angular not covered) | Partially Synced — only Light-theme/idle instance sourced, no Dark-theme or mid-interaction evidence; track width rounded 54px→56px so thumb travel lands on a real spacing token; React/WC/Angular parity complete | 2026-07-20 |
| Page Header | `1007:3700` canonical `1119:3341`, re-verified against example `1197:1652` (`1102:6519`) | `packages/ui/src/composite/PageHeader.tsx` | `Composite/PageHeader` | Partially Synced — breadcrumb/description typography rounded to nearest existing tiers; breadcrumb link color corrected gray→blue (`text.link-subtle`) against the canonical instance; breadcrumbs render inline rather than composing the separate Breadcrumb spec (§22 of component-specifications.md); React only, no cross-framework equivalent expected (composite/page-level) | 2026-07-20 |
...
| AppShell (`rail` variant) | `1007:3700` (canonical `NavigationRail` component `1079:2686`, re-verified against `1197:1652`) | `packages/ui/src/layout/AppShell.tsx` | `Layout/AppShell` (`Rail` story) | Synced — canonical `NavigationRail` component matches the shipped implementation exactly, no changes needed on re-audit; only closed/collapsed rail sourced, no expanded-rail or open/close transition instance | 2026-07-20 |
| AppShell (`sidebar` variant) | `1007:3700` (canonical `SideNav/Expanded` component `1079:2427`, `Breakpoint=Desktop/Theme=Light` instance `1127:4196`) | `packages/ui/src/layout/AppShell.tsx` | `Layout/AppShell` (`Playground` story) | Synced — rebuilt 2026-07-20 from a prior implementation that predated this repo's Figma-sync discipline and matched no sourced evidence; **breaking**: `nav` changed `NavItem[]`→`NavSection[]`, see `docs/component-architecture.md` | 2026-07-20 |
| AI Panel | `1007:3700` (`1079:3141`, instance `1119:3351`) | `packages/ui/src/composite/AIPanel.tsx` | `Composite/AIPanel` | Partially Synced — new component; header icon substituted (`LmAisymbolIcon` for Figma's unavailable `lm-ai-outline`); live-region screen-reader behavior not independently verified; React only | 2026-07-20 |
| Button (`accent` variant) | `1007:3700` (`1127:4196`, `btn/accent/*` variables) | `packages/ui/src/primitives/Button.tsx`, `packages/web-components/src/button/lumen-button.ts`, `packages/angular/src/button/lumen-button.ts` | `Primitives/Button` (`AllVariants` story) | Partially Synced — only Default state sourced, hover/active/focus are an undocumented-in-Figma placeholder; React/WC/Angular parity complete | 2026-07-20 |
