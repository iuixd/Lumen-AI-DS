...
- [x] `Calendar` and `Chart` adopted as batch 6. First date-handling (`react-day-picker`, `date-fns`) and charting (`recharts`) dependencies. `Chart` composed with a `dataviz`-skill-validated categorical palette (`chartCategoricalColors`, computed via `validate_palette.js`, not eyeballed) rather than shadcn's default per-series scheme, since Lumen has no chart color tokens of its own — see `docs/shadcn-integration.md` §7.7. This closed the original ~61-item bulk-adoption plan (§7): every component from that set had been adopted, dropped as genuinely unavailable for this repo's `new-york` registry style, or skipped by direct user decision (`Sidebar`, `Message`/`MessageScroller`/`Bubble`).
- [x] `Menubar`/`ResizablePanelGroup`+`ResizablePanel`+`ResizableHandle`/`InputGroup` adopted as batch 7, opened 2026-07-24 by a fresh, explicit 28-name request covering components that didn't exist (or weren't requested) in batches 1–6 — see `docs/shadcn-integration.md` §7.9. `Empty`/`Field` skipped as full functional duplicates of `EmptyState`/`FormField`; 10 of the 28 names still don't resolve as installable shadcn components. Caught two real upstream-version bugs in `react-resizable-panels@4.12.2` (renamed exports, pixel-vs-percent size semantics) and one `cn()`-merge-order bug in `InputGroupButton`. Alongside this batch, a full shadcn functionality/behavior/API parity audit of all 13 previously-adopted components found real drift only in `Command` (the first component ever adopted, predating later conventions) — see `docs/shadcn-integration.md` §7.10 and `docs/changelog.md`'s Fixed entry.
...
- [x] `lm-bot-animated` added 2026-07-27 — supersedes the deferral below. Figma's `get_motion_context` never did produce usable keyframe data (see the original deferral reasoning, kept for the record), but the user supplied the animated SVG directly (antenna wiggle, eye-blink, loading-dot mouth animation, all via embedded CSS `@keyframes` on a 6s loop) — no `motion/react` dependency needed, since the animation is plain CSS baked into the SVG itself, not a React-driven timeline. The icon pipeline (`icons-import.mjs`) had never ingested an SVG with an embedded `<style>` block before; two real, generic compile issues surfaced and were fixed there (not patched onto the generated output): JSX doesn't allow literal `{`/`}` as an element's raw text children (the `<style>` tag's CSS), and React's JSX `style` attribute must be an object, not a string (SVGO had moved each animated element's rule from an ID-selector to an inline `style="..."` attribute). See `docs/changelog.md`'s Added entry for the full record, including a new `AIPanelMessage.avatarIcon` override this icon is used through.
  - Original deferral (2026-07-24), superseded above: Figma's `get_motion_context` returned only an inventory (9 layers in one 6.36s looping timeline cohort) with empty keyframe/transition values for all of them, even after the user renamed and re-checked the node — there was no real animation data to reproduce faithfully via that route, and building it via `motion/react` would have needed a new runtime dependency (a bigger, undiscussed decision). Resolved instead by the user supplying the actual animated asset directly.
