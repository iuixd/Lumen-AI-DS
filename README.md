...
- Node.js 22
- pnpm 11.20.0, selected through Corepack
...
The result must be `11.20.0`. Avoid using a globally installed incompatible pnpm version.
...
the pinned `11.20.0` regardless of what pnpm install, if any, is also on `PATH` —
...
pnpm` can resolve the newer cached version instead of `11.20.0`, and pnpm refuses to
continue with an error like:

```text
This project is configured to use 11.20.0 of pnpm. Your current pnpm is v11.21.0.
